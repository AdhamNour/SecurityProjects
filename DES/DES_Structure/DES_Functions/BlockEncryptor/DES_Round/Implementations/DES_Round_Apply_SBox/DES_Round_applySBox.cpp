...
void DES_Round::applySBox(string& target){
    ...
    for (int i = 0; i < target.length(); i+=6)
    {
        ...
        int resultInt = SBoxes[SBox_index][dataConverter.convertFromBinary2Decimal(rowIndex)][dataConverter.convertFromBinary2Decimal(rowIndex)];
        result+=dataConverter.convertFromDecimal2Binary(resultInt).substr(4);
    }
    if(result.length() != 48){
        cerr << "There's error occured in \"apply SBox\" the result length isnot 48"<<endl;
        return;
    }
    target = result;    
}